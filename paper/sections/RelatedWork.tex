...
\item Mixins allows to name components that can be applied to various classes as
  ...
  To fight those limitations, an algebra of mixin operators is
  introduced~\cite{ancona2002calculus}, but this raised the complexity of the
  approach, especially when constructors and fields are
  considered~\cite{marco09FJigsaw}.
...
\item Simplifying the mixin algebras approach, traits draw a strong line between
  ...
  of state as drawbacks of this model.
...
\marco{text under still to revise. Sould all the lombock discussion go in the implementation section? -- just to keep stuff together?}

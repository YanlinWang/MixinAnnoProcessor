...
Java.
In their formalization, classes and interfaces can have only one method \Q@m()@ without arguments, so as to simplify method overloading and renaming.
Our work expands and generalizes the situation to support multiple methods with some arguments inside interfaces, in which case the methods that \mixin generates are better modelled. Meanwhile, features like multiple inheritance of interfaces and reabstraction of default methods are also preserved in our formalization. Furthermore, one advantage of the \Q@of@ method provided by \mixin is that it's generated as a static method, considered as a clean way without affecting its heirs. It is indeed supported since static methods are included in our formalization.
\bruno{explain why having multiple methods is significantly
  more challenging.}
%Our code generation generates several methods that may

...
problem~\cite{bracha90mixin,Sak89dis}, conflicting methods, etc.) in reasoning
...
virtual inheritance aims at a relative general model; mixin model add some
...
class hirarchy complexity increases. \mixin does not complicate the
...
(multiple) traits. Java 8 interfaces with default methods are closely related to
...
with operations like sum, alias and exclusion, providing explicit conflict
...
the new fork gains enough traction to become the main language release. % On this
% topic we mention
...
purpose of SugarJ is to allow easy defition of sugars while our approach aims to
implement the desired program transformation without just in Java.

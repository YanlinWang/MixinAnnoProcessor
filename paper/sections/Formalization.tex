...
In the definition we denote by $\Aux{definedMethod}(\m,\C)=\method$ the non-static method called $\m$ defined directly in $\C$.
...
\InTextDef{43ex}{
\Aux{definedMethod}(\m,\C_0)\in\tops(\m,\C_0\dots\C_n)}{
 }{}\\
\tab\tab
\not\exists i\in1..n \mbox{ such that } 
\Aux{definedMethod}(\m,\C_i) \mbox{ is defined and }
\C_i \subtype \C_0

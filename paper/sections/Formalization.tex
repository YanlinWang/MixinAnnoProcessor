...
($\C\QM{.super.}\m\QM(\es)$), whose semantic is to call the (non-static) method $\m$ over the $\this$ receiver, but statically
...
For simplicity, we do not model overloading, thus for an interface to be well formed its methods must be uniquely identified by their name.
...
\noindent The definition of $\mBody$ reconstruct the full set of supertypes $\Cs$ and then delegates the work to two others auxiliary functions:
 $\tops(m,\C_0)$%, $\shadow(\methods)$
 and $\override(\method,\method')$.

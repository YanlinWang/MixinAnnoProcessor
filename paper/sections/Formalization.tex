...
\paragraph{Example}
To illustrate the mechanism of $\mBody$ in a more intuitive way, we present an example here.
\begin{lstlisting}
...
\item To compute $\tops(\QM{m},\{\QM{A,B,C}\})$, $\QM{C}$ will be omitted, since $\QM{m}$ is not declared directly in $\QM{C}$, hence $\Aux{originalMethod}(\QM{m},\QM{C})$ is undefined. And $\QM{B}$ is a subtype of $\QM{A}$ (thus $\QM{B.m}$ hides $\QM{A.m}$), so $\tops(\QM{m},\{\QM{A,B,C}\})$ equals $\QM{B.m}$.
\item The final step is to compute $\override(\QM{D.m},\QM{B.m})$, and this matches the fifth case of $\override$ that $\QM{D.m}$ hides $\QM{B.m}$ successfully ($\QM{String}$ is a subtype of $\QM{Object}$) and becomes the most specific method. So finally we get $\mBody(\QM{m},\QM{D}) = \QM{D.m}$.
\end{itemize}

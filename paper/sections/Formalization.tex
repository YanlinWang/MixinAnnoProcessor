...
Before formalizing our annotation, we summarize methods interaction/shadowing 
mechanism in Java 8 interfaces and especially. We show several interesting cases
...
From Java 8 interfaces can have three type of methods: abstract, default, and
static methods.
\begin{itemize}

\item Static methods are handled in a very clean way: they are visible only in
  the interface they are explicitly defined. This means the following code
  is ill-typed.
\begin{lstlisting}
...
\item Abstract methods composition is accepted when exist a most specific one.
...
\item Default methods conflict with any other default or abstract method. For
  example the following code is rejected due to methods conflict.
\begin{lstlisting}
...
\item A method in the current interface wins over any method in its
super-interfaces, provided that the method%  conforms to the subtype of all methods
% in its super-interfaces, i.e., the method
...
\Q@B@${}_i$? The 9 cases are shown here:\\*
\noindent
...
\begin{itemize}
\item \textbf{conflict error} happens when the methods from \Q@A@${}_i$ and
  \Q@B@${}_i$ are both implemented, which is also an error in most trait models.
\item \textbf{both abstract, accepted} happens when the methods from \Q@A@${}_i$
  and \Q@B@${}_i$ are both abstract, which is also considered correct in all
  trait models.
\item \textbf{conservative error} happens when only one method from \Q@A@${}_i$
  or \Q@B@${}_i$ is implemented, which is different from what we would expect in
...
During our experimentation, we found a bug in ECJ (Eclipse compiler for Java):
...
rejection, hence this is a bug in ECJ.\footnote{This bug was also reported by
  others and is fixed in the ECJ developer brunch, but not released as a stable
  version yet.}
...
For simplicity, we do not model overloading, thus for an interface to be well formed its methods must be uniquelly indentfied by their name.
...
\InTextWith{
 \method\mbox{ not a static method, and }

...
Before formalizing Classless Java, it is helpful to informally discuss the behaviour
of methods in Java 8 interfaces.
...
We illustrate the possible outcomes using the following (correct) declarations:
\begin{lstlisting}
interface A1{T m(); }
...
\Q@B@${}_j$? Nine representative cases are shown next:\\*
\noindent
...
\begin{itemize}
\item \textbf{conflict error} happens when the methods from are both implemented, which is also an error in most trait models.
\item \textbf{both abstract, accepted} happens when the methods from are both abstract, which is also considered correct in all
  trait models.
\item \textbf{conservative error} happens when only one method from is implemented, which is different from what we would expect in
...
\noindent Here we require that for all newly declared methods, there is a
...
\noindent The definition of $\mBody$ delegates the work to three others auxiliary functions:
...
the non-static ones that are not transitively reachable by following another, less specific, superinterface chain.
...
\rn{t-intf} relies on this behaviour.

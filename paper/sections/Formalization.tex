...
\forall i\in 1..n\ \ \Gamma \vdash \e_i \in \_<:\C_i \\\\
  \textsf{mtype}(\m,\C) \!=\! \C_1\ldots\C_n \!\!\to\! \C'
%\textsf{mmodifier}(\m,\C) \neq \textbf{static}
 }{
...
\forall i\in 1..n\  \ \Gamma \vdash \e_i\in \_<:\C_i \\\\
\textsf{mtypeS}(\m,\C) \!=\! \C_1\ldots\C_n \!\to\! \C'
%\textsf{mmodifier}(\m,\C) = \textbf{static}
}{
\Gamma \vdash \C\QM.\m\QM(\e_1\ldots\e_n\QM) \in \C'}
\quad\quad
...
}{\Gamma \vdash \C\QM.\QM{super}\QM.\m\QM(\e_1\ldots\e_n\QM) \in \C'}
...
method with the same name defined in the interface $\C$, and that such method is a subtype of the newly introduced one. We formally define here subtyping between method, in a more general form that will be useful later.
...
That is, we allows return type specialization as introduced in Java\marco{insert the version number}.
...
\InTextAssert{25ex}{
\method\in\tops(\m,\Cs)}\\
\InTextWith{\C\in\Cs\ ,\method=\mBody(\m,\C), \method\mbox{ not a static method and } \nexists \C'\in\Cs\setminus\C \mbox{ such that } \C' \subtype \C}
\end{array}$
...
Since method subtyping is a partial ordering, this may be not defined, this in turn makes \shadow{}, and the whole \mBody{} not defined for that specific $\m$. Rule  \rn{(t-intf)}  rely on this behaviour.
...
\noindent$\begin{array}{l}
\InTextDef{18ex}{\override(\none,\none)}{\none}\\
\InTextDef{18ex}{\override(\method,\none)}{\method}\\

\InTextDef{18ex}{\override(\none,\method)}{\method}\\
\InTextWith{\method\mbox{ not of the form } \conflicted\ \mh\QM;}\\
\InTextDef{18ex}{\override(\method,\method')}{\method}\\

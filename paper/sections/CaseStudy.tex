...
evolution. Recently, a new solution~\cite{eptrivially}, \marco{repharesed} using only
covariant type refinement, has been shown.
When such solution is modelled with interfaces and default methods, it can
even provide independent extensibility: the ability to
assemble a system from multiple, independently developed, extensions.
Unfortunatelly, the required instantiation code makes a plain Java solution verbose and cumbersome to use. The \mixin annotation is enough to remove the boilerplate code, making the presented approach very appealing.
...
\noindent As shown in Figure~\ref{fig:ep}, \texttt{Exp} is the common
...
\end{minipage}
\end{tabular}
\caption{The Expression Problem. Initial system on the left, and code for adding
  print operation on the right. }\label{fig:ep}
\end{figure}

\paragraph{Adding a New Type of Expressions}
In the object oriented paradigm, it is easy to add new types of expressions. For example, the following code shows how to add subtraction.
...
The code in Figure~\ref{fig:ep} shows how to add the new operation \texttt{print}.

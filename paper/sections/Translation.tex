...
\haoyuan{In theorem: $I_0$ is valid and $\QM{of}\notin\dom{I_0}$. To prove the theorem we need an extra lemma: if the translation is well defined, then $[\![\II_0]\!]$ is well formed.}
...
\begin{itemize}
\item \textit{Self coherence}: the generated code itself is well-formed; there are no type errors.
\item \textit{Client coherence}: the client code doesn't fail if it was not failing before code generation.
\item \textit{Heir coherence}: the heirs are safe. 
\end{itemize}
...
Here is the client code for our example. With interface \Q@B@ annotated with \mixin, by the translation rule, it will generate a method ``\Q@B withX(int x);@'' inside, as an automatic return type refinement. But in that case, interface \Q@C@ doesn't type check.

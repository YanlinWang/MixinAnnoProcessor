...
\hl{To translate an interface annotated by \mixin, we add some other methods for type refinement, then add a new annotation \textbf{@ObjWeak} to the interface.}
...
\InTextWith{\valid(\C_0),\QM{of}\notin\dom(\methods) \mbox{ and } \methods'=\ofMethod(\C_0)}
\end{array}$

\hl{To translate an interface annotated by \textbf{@ObjWeak}, we add the $\QM{of}$ method.}

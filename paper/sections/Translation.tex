...
\subsection{Guaratees}
Now we try to understand what is the meaning of saying that our approach is correct, and more in general, what it means to say that a language tuning is correct.\haoyuan{Actually only self coherence is supported?}
We identify three types of guarantees:
\begin{itemize}
...
\mixin  does not guarantee \textit{Client coherence}.
...
\noindent By the translation rule, \mixin would  generate in \Q@B@  a method ``\Q@B withX(int x);@''.
This would break \Q@C@. And it's similar why client coherence is not supported as well. In the client code, an expression
that uses ``\Q@new B(){...}@'' to create an object of \Q@B@ gives an implementation of \Q@A withX(int x);@, but after the return
type is refined to \Q@B@, there is a type error.
\end{itemize}

\noindent Consequently, type refinement is a common case in client code, and hence is included by \mixin as a feature for convenient use, whereas it simultaneously brings some risk in throwing type errors, though they are likely to be fixed easily by users. On the other hand, a property of \mixin is informally concluded as: for a client program that typechecks before a translation is applied, if the annotated type has no subtypes, and no objects of that type are created, then type safety is guaranteed after the successful translation. Conversely, the second step of \mixin, namely what \Q:@ObjWeak: does in the formalization, is guaranteed to be type-safe for the three kinds of coherence. The formal theorem and proof is presented afterwards.

\subsection{Results}

\haoyuan{We use \textbf{@ObjWeak} or \Q:@ObjWeak:?}

The theorem is presented below. Lemmas and the complete proof are available in Appendix~\ref{subsec:lemma1},~\ref{subsec:lemma2} and~\ref{subsec:theorem}.\\

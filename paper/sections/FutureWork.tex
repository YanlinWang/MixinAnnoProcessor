...
package private (java default) visiblity.

We can add a second annotation 
\Q!@Exposed!
that leverage on this edge:
An
interface without exposed works as usual, but if any method of a public \mixin interface is annoted with \Q!@Exposed!, we can apply a translation where a new  (package private) interface type is introduced, and the orginal interface become just a facade.
For example:
\begin{lstlisting}
...
\end{lstlisting}
 ...
 This is not a perfect solution, since \Q@Person$@ can still be seen inside the \Q@Person@ package and heirs of \Q@Person$@, however it is surprising we achive such of a good result without any language support for privatness in interfaces.
...
\end{comment}
\marco{old version still commented if we decided to swap back}

...
\end{figure}
\bruno{Yanlin: figure needs to rename point to location}

This section illustrates how the \mixin annotation enables expressive
idioms using multiple inheritance. 
...
Before we model the final animal system, we first start with a
...
\noindent There is a some annoyances here. Firstly, the sole
...
\lstinputlisting[linerange=9-15]{../UseMixinLombok/src/pegasusLombok/Main.java}% APPLY:linerange=PEGASUS_LOMBOK
...
provide an alternative implementation using \Q@default@ methods:
\begin{lstlisting}
@Obj interface Point2D{ 
  int x(); int y();
...
implementation is a new type of \@with@ method, called a property updater.
...
\noindent Note that this interface includes a different type of
\Q@with@ method, which takes a \Q@Point2D@ as an argument.
The old wither methods (such as \Q@withX@) functionally update a field at a time.  This can be
...
updated. The idea is that we want the result to still be like \Q@this@, but
...
generated code is very mechanical, writing such code by hand is error-prone. For

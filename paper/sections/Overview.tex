...
\begin{figure}
\begin{tabular}{c|c}
\includegraphics[height=3.5cm]{PegasusDetail.pdf}\hspace{20pt} &
\begin{minipage}{7cm}
\vspace{-90pt}
...
\end{figure}
\bruno{Yanlin: We need space, but we cannot make the figure {\bf unreadable}; the
  diagram fonts are too small now. Please increase the size of the
  picture a bit. You can also use more space in the Java code.}

This section illustrates how the \mixinAnn{} annotation enables expressive
...
justify the \mixinAnn{} annotation. Lets now move on to modelling 
the complete animal system, where \Q@Animal@s will include a \Q@location@, representing their position in space. As we shall
...
\noindent We can provide implementation for methods; 
we generate code only for methods needing implementation. Thus, programmers can easily tailor the behaviour on their special needs.
%\end{comment}
...
We are tempted to assume that a language extension is needed. 
%Instead, with object interfaces, another approach is possible
Instead, with our language \emph{restriction}, another approach is possible;often in programming languages ``freedom is slavery''.
...
updated. The idea is that the result should be like \Q@this@, but

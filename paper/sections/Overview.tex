...
\marco{fig1 is now better. Where we cite it in the text? can we make it so that is more "linear"? point2d <- point3D,   animal <- (horse over bird ) <- pegasus}
...
\lstinputlisting[linerange=6-13]{../UseMixinLombok/src/pegasusJava/Main.java}% APPLY:linerange=PEGASUS_JAVA
...
\bruno{
Note that \texttt{Bird} can fly, therefore refines the
type of \texttt{point} from \texttt{Point2D} to
\texttt{Point3D}. 
}
...
\lstinputlisting[linerange=17-19]{../UseMixinLombok/src/pegasusJava/Main.java}% APPLY:linerange=PEGASUS_INST
...
system presented in Figure~\ref{}, and still does not appear to 
...
However programmers are not required to use this cumbersome syntax for
...
%\lstinputlisting[linerange=50-56]{../UseMixinLombok/src/test/TestAnimal.java}% APPLY:linerange=POINT2D
...
\lstinputlisting[linerange=68-71]{../UseMixinLombok/src/test/TestAnimal.java}% APPLY:linerange=ANIMAL
...
\lstinputlisting[linerange=75-78]{../UseMixinLombok/src/test/TestAnimal.java}% APPLY:linerange=HORSE
...
as follows:
\lstinputlisting[linerange=60-64]{../UseMixinLombok/src/test/TestAnimal.java}% APPLY:linerange=POINT3D
\marco{Fix over: with(Point must be with(Point2D
and it should be after withZ, and on more lines}
...
\subsubsection*{Unsatisfactory class based solutions to field type refinement}
...
\lstinputlisting[linerange=82-90]{../UseMixinLombok/src/test/TestAnimal.java}% APPLY:linerange=BIRD
...
\lstinputlisting[linerange=98-99]{../UseMixinLombok/src/test/TestAnimal.java}% APPLY:linerange=PEGASUS

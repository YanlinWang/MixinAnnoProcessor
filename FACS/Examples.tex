...
formally. \bruno{Could we point out here a few references of key 
ideas that were presented in other work? This would justify the 
formalization sketch better, I think.}
...
trait composition operator~\bruno{Maybe some references here would
  help}.  

$\use\ \overline\mV$ composes the content of $\overline\mV$
by taking the union of the methods and the union of the
implementations.\bruno{Also, for Use, wasn't this similar to your
  previous work in FJig/DeepFJig?}
...
While this seems a dangerous approach at first, consider that also Java have the same behaviour:
for example in
...
literals have to be \emph{self complete}. In this case we would have been forced to

...
In our simple mode\bruno{you mean model?}, we consider an error trying to merge a class and an interface.
...
\bruno{ok: would be nice to see an example?}
...
\item \bruno{method conflict detection in traits?}
...
other constructs~\bruno{don't you actually have evidence that this is
  the case from 42?}. We use minimal expressions to keep our example
...
knowing itself and \Q@IA@.\bruno{why IA? it is not used in this
  example, right?}
...
declare abstract methods \Q@n@ and \Q@m@.\bruno{right. this is a good point}

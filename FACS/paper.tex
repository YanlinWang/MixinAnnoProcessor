...
\begin{abstract}
In most object oriented languages subclassing/code-reuse implies
...
\emph{leaking problem}. The leaking problem shows that the separating
...
This paper synthesizes an OO language design for a nominally typed
language that allows code-reuse and subtyping to be decoupled using a
form of \emph{traits}. This design improves on previous work by
Damiani and Servetto by providing a simpler and more natural model for
OO programmers. By making our type system distinguish between code use
and code reuse we can avoid many redundant abstract declarations that
were required in previous work by Servetto and Damiani, while
supporting self construction, binary methods and recursive
types. Moreover, we provide a novel and elegant solution to uniformly

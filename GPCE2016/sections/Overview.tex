...
Here we will illustrate how points are modelled with interfaces.
 In IB
...
\noindent The encoding over pure Java is now inconvenient: creating a new point object is cumbersome, even
...
%names).
 An example of using \Q@Point2D@ is:
\begin{lstlisting}
...
\paragraph{Property updater as Injections}

By declaring fields in Newspeak and other languages,
the programmers specifies field signatures and (critically) the intention of storing such information. Then the language generate getter and setter.
Once state is abstracted away,
it is well known that is possible to replace state
access with computation, but the type of the field stays the same.

Our fluid approach to state is specular:
the intention of storing the information is not expressed by the programmer and set in stone, 
but can vary by inheritance:
We generate the idea of the field
just starting from getters and setters signatures.

In this case, we leverage on our fluid state to changing the underling type of the field,
 and our \Q@with@ is providing
the right injection from the old type to the new one.

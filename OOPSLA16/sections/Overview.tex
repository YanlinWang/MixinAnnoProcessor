...
This section illustrates how our programming style, supported by the \mixinAnn{} annotation, enable powerful
idioms similar to multiple inheritance and field type refinement.
We propose a standard example: \Q@Animal@s with a two-dimensional \Q@Point2D@ representing their \Q@location@.
Some kinds of animals are \Q@Horses@ and \Q@Bird@s.
Birds can \Q@fly@, thus their locations need to be three-dimensional \Q@Point3D@s (field type refinement).
Finally, we model \Q@Pegasus@ (one of the best-known creatures in Greek
mythology) as a kind of \Q@Animal@ with the skills
of both \Q@Horse@s and \Q@Bird@s (multiple inheritance). A simple class diagram illustrating
...
Here you still can not appreciate the advantage of IB.
Lets model now the complete animal system where \Q@Animal@s will include a \Q@location@
representing their positions in space. We will use 2D points to keep track of locations using coordinates.

\paragraph{\Q@Point2D@: simple immutable data with fields}
Here we will illustrate how points are modelled with
interfaces. In IB we do not talk about state directly; state is accessed
using abstract (getter) methods. \bruno{unfinished sentence: Note how in}   The usual
approach to model points in Java is to use a class with fields for the
coordinates.
...
\noindent Our encoding over Java is now inconvenient: creating a new point object is cumbersome, even
with anonymous classes:

\begin{lstlisting}
Point2D p = new Point2D() {
  public int x(){return 4;} public int y(){return 2;}  }
\end{lstlisting}
...
can be synthesized can be \mixin. A non \mixin interface is like an abstract class in Java.
%\end{comment}
...
relies on a notion of abstract state:
\marcoT{Method bodies can only refer to other methods;
Objects are (conceptually) closures and the state is 
composed by captured local variables.}
Methods interacting with those are called \emph{abstract state operations}.

Object interfaces provide support for four
different types of abstract state operations \textcolor{red}{together with two types of
general operations}, which are summarized in
...
\multirow{4}{*}{State operations} & \textbf{``fields''/getters}        &   \Q@int x()@                  & Retrieves value from field \texttt{x}.          \\ \cline{2-4}
...
%\end{tabular}
 & Generate factory method for \texttt{Point2D}.        \\
\cline{2-4}
& \textbf{functional updaters}        & \Q@Point3D with(Point2D val)@ & Update all fields
that match fields in \texttt{val}.        \\
\cline{2-4}
& \textbf{imperative updaters}        & \Q@Point3D set(Point2D val)@ & Set all fields
that match fields in \texttt{val}.        \\ \hline
\end{tabular}

\caption{\textcolor{red}{(edited2)} Abstract state operations for a field \texttt{x}, together with general operations, supported by the \mixin
  annotation. }
...
be refined. Such kind of refinement is challenging
...
\bruno{At some point we should talk about the point that was discussed in the reviews}
\marco{you mean the reviewer question on the list.size?}
...
% fields of another interface.
 ...
 \marcoT{Symmetrically,
 we could offer an imperative property updater
 that calls the setters instead of the withers.
 \Q@Point3D set(Point2D val)@.
 }
 
 \marcoT{
 Property updaters never break class invariants, since they
 internally call operations that was already deemed
 safe by the programmer. For example a list object
 would not offer a setter for its size field, thus
...
\begin{figure}
\marco{I can not find the declaration int x();
also we can change the order of the code to write the non generated stuff first.}
\saveSpaceFig

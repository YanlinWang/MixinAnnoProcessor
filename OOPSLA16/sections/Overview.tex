...
\multirow{2}{*}{Other operations} & \textbf{factory methods} & \begin{tabular}{l}\Q@static Point2D of(int _x,@\\ \hspace{.8in}\Q@int _y) {...}@\end{tabular} & Generate factory method for \texttt{Point2D}.        \\ \cline{2-4}
& \textbf{general updaters}        & \Q@Point3D with(Point2D val);@ & Update all fields
that match the object \texttt{val}.        \\ \hline
\end{tabular}

\caption{\textcolor{red}{(edited)} Abstract state operations for a field \texttt{x}, together with general operations, supported by the \mixin

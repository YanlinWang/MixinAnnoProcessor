...
encoded in Classless Java. The second application. shows how to model an
embedded DSL based on fluent APIs. The two larger case studies, take
...
second case study maintains the same amount of code, but increases
modularity.

\subsection{The Expression Problem with Object Interfaces}
...
\subsection{Refactoring an Interpreter}
...
which is created for tutorial purpose of Truffle~\cite{wurthinger2013one}. Keeping a balance between
...
interfaces, separate out \texttt{eval} and \texttt{print} operations then combine

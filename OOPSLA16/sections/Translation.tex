...
This section gives an overview of what $\mixinAnn$ generates in
Classless Java, and what properties are guaranteed in the translation.

\marcoT{We formalize syntax and typing for
...
\marcoT{Since our properties are about preserving typing,
we do not need to formalize Classless Java semantic to proof our statements.}
\begin{figure*}[t]
\centering
\hspace{-.2in}
...
functions. \bruno{if the figures are mentioned here, what should
  follow is the explanation on what is one the figures. Please fix
  this Haoyuan and makes sure that everything that is on the figures
  is explained here. }
\haoyuan{In Figure~\ref{figure:valid}, the $\otherMethod$ function is defined in the first two
equations. This function generates unimplemented \Q@with-@ and fluent setters in the interface.
They are generated as abstract methods because the return types need to be refined.
To determine whether the methods need to be generated,
we check if such \Q@with-@ or setter methods
are required by $\C_0$, but not declared directly in $\C_0$. The third equation gives the definition
of $\valid$. To generate the \Q@of@ method, it is necessary for $\weakAnn$ to explicitly check if all
abstract methods are valid. That is, we can categorize them in a pattern that we
know how to implement: it is either a field getter, a with method or a setter. Auxiliary checkers are
defined on the right side. Note that we write $\QM{with#}\m$ to append $\m$ to
...
In Figure~\ref{figure:ofmethod}, the function $\ofMethod$ generates the static method $\QM{of}$, as an
object factory. It detects all the field methods of $\C_0$, take them
as arguments, and in the return statement is an anonymous class which
generates getters, fluent setters and with-methods in certain
cases. Some other features of $\mixinAnn$, including non-fluent
setters and the $\QM{with}$ method, however, are not formalized in
Classless Java. Appendix~\ref{subsec:otherfeatures} gives an informal
explanation of those methods.

\bruno{I think there is explanation missing in this subsection;
  probably text which is now in the appendix. Haoyuan, please take
  care of this.}\haoyuan{fixed}
...
\bruno{Do we have good names for the theorems Marco? Cannot be called
  tunning now!}
\begin{thm}[@ObjOf]
...
materials\bruno{point to appendix instead}.
...
\bruno{I don't
  understand the point of the last sentence? Rephrase?}
  \marco{ok I tried to rephase the next sentence, how is now?}
